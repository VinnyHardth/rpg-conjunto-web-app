  ...
  return (
    <div className="text-center p-8 bg-gray-50 rounded-lg shadow-inner ">
      ...
      <div className="inline-block text-left p-6 bg-white border border-gray-200 rounded-xl shadow-lg mb-6 mr-4">
        ...
        <div className="grid grid-cols-2 gap-x-12 gap-y-3 text-base">
          ...
          <div>
            ...
            {characterData.info.age || "—"}
          </div>
          <div>
            <span className="font-semibold">Altura:</span>{" "}
            {characterData.info.height ? `${characterData.info.height}cm` : "—"}
          </div>
          {characterData.archetype.name && (
            <div>
              <span className="font-semibold">Arquétipo:</span>{" "}
              {characterData.archetype.name}
            </div>
          )}
        </div>
      ...
      <div className="inline-block text-left p-6 bg-white border border-gray-200 rounded-xl shadow-lg mb-6">
        ...
        <div className="grid grid-cols-2 gap-x-12 gap-y-3 text-base">
          <div>
            <span className="font-semibold text-red-600">HP Máximo:</span>{" "}
            <span className="font-bold">{derivedStats.hp}</span>
          </div>
          <div>
            <span className="font-semibold text-blue-600">MP Máximo:</span>{" "}
            <span className="font-bold">{derivedStats.mp}</span>
          </div>
          <div>
            <span className="font-semibold text-yellow-600">TP Máximo:</span>{" "}
            <span className="font-bold">{derivedStats.tp}</span>
          </div>
          <div>
            <span className="font-semibold text-green-600">Movimento:</span>{" "}
            <span className="font-bold">{derivedStats.movimento}m</span>
          </div>
        </div>
      </div>
